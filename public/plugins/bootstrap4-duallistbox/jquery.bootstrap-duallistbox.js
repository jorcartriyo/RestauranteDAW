...
(function (factory) {
    ...
    var pluginName = "bootstrapDualListbox",
        defaults = {
            ...
            infoText: "Roles {0}", // text when all options are visible / false for no info text
            infoTextFiltered:
                '<span class="badge badge-warning">Filtrados</span> {0} from {1}', // when not all of the options are visible due to the filter
            infoTextEmpty: "Lista vacía", // when there are no options present in the list
            infoTextEmpty2:
                "Esta lista no Puede quedar vacía. Debe de dejar algún rol", // when there are no options present in the list
    ...
    function remove(dualListbox) {
        ...
        dualListbox.element
            .find("option:selected")
            .each(function (index, item) {
                seleccion.push(item.value);
            });
        let pasa = false;
        pasa = seleccion.includes(seleccionado);
        dualListbox.elements.select2
            .find("option:selected")
            .each(function (index, item) {
                var $item = $(item);
                //Cambiadlogo  dualListbox.selectedElements > 1
                if (
                    !$item.data("filtered2") &&
                    dualListbox.selectedElements > 1
                ) {
                    changeSelectionState(
                        dualListbox,
                        $item.data("original-index"),
                    ...
                    content = dualListbox.settings.infoTextEmpty2;
                    dualListbox.elements.info2.html(content);
                }
            });
    ...
    function removeAll(dualListbox) {
        ...
        //Cambiado  && dualListbox.selectedElements > 1;
        dualListbox.element.find("option").each(function (index, item) {
            var $item = $(item);
            if (!$item.data("filtered2") && index > 0) {
